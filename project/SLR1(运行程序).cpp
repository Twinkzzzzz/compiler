...
string l,t,mcode,arcode[10100];
int i,j,sp,ip,arip,pos,pt,kt,inputlength,flag,state[1010],dime[1010][50],varnuminfunc[1010],labelnum=0,varnum=0,vartype=0; //ip输入串指针,sp栈指针 
...
void genactionrecord(string funcname,string uppername)
{
	arip++;
	arcode[arip]="\n//"+funcname +"(start)";
	...
	arip++;
	arcode[arip]="sp++;";
	int pos1=globaltab.look(uppername);
	int nlayer=globaltab.nametab[pos].l_o.lev;
	int dlayer=globaltab.nametab[pos1].l_o.lev;
	if (nlayer==dlayer+1)
	{
		arip++;
		arcode[arip]="M[sp]=fp;";
	}
	else if(nlayer==dlayer)
	{
		arip++;
		arcode[arip]="M[sp]=M[fp-1];";
	}
	else
	{
		arip++;
		arcode[arip]="nt=fp;";
		arip++;
		arcode[arip]="for(i=0;i<="+inttostring(dlayer-nlayer)+"i++)";
		arip++;
		arcode[arip]="{";
		arip++;
		arcode[arip]="nt=nt-1;";
	...
	arcode[arip]="sp++;";
	arip++;
	arcode[arip]="M[sp]=code@address;";
	int mv=globaltab.count_par(funcname)+globaltab.count_real(funcname)+1;
	arip++;
	arcode[arip]="sp=sp+"+inttostring(mv)+";";
	kt=globaltab.btab[pt].last;
	while(kt!=-1)
	{
		arip++;
		arcode[arip]="sp--;";
	...
	{
		arip++;
		arcode[arip]="sp=sp+"+inttostring(mv-2)+";";
	}
	else if(mv==1)
	{
		arip++;
	...
	arip++;
	arcode[arip]="fp=M[fp];";
	mv=globaltab.count_par(funcname)*2+globaltab.count_real(funcname)+3;
	if(globaltab.nametab[pos].type!=0) mv++;
	arip++;
	arcode[arip]="sp=sp-"+inttostring(mv)+";";
	return;
}
...
void printarcode()
{
	freopen("actionrecordcode.out","w",stdout);
	for(int i=1;i<=arip;++i) cout << arcode[i] << endl;
	return;
}
...
int main()
{
	...
	while(ip<inputlength)
	{
		...
		else if(!act.first)
		{
			...
			arcode[arip]="\n//global";
			arip++;
			arcode[arip]="M[0]=NULL;";
			arip++;
			arcode[arip]="M[1]=NULL;";
			arip++;
			arcode[arip]="M[2]=0;";
			arip++;
			arcode[arip]="fp=1;";
		...
		else	
		{
			switch(act.second)
			{
				...
				{ 
					...
					sp-=4;
					genactionrecord(prop[sp],tmpfunc.top());
					break;
				} 
				case(21):
				{
